...
class AddCreatedAtInViewTicketsDetailed extends Migration
{
    /**
     * Run the migrations.
     *
    ...
    public function up()
    {
      ...
      SELECT
          ...
          `ttr`.`title` AS `ticket_subject`,
            ...
            `tic`.`updated_at` AS `last_activity`,
            `tic`.`created_at` AS `first_activity`,
      ...
      FROM
          ...
          INNER JOIN `ticket_thread` AS `ttr` ON (`tic`.`id` = `ttr`.`ticket_id`)
        ...
        GROUP BY `tic`.`id`
      ORDER BY `tic`.`id` DESC, `ttr`.`created_at` DESC
      ");
