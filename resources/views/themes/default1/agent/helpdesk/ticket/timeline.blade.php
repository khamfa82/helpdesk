...
if ($thread_title != "") {
    $title = wordwrap($thread_title, 70, "<br>\n");
} else {
    $title = "";
}
?>
...
<div class="box box-primary">
    <div class="box-header">
        <h3 class="box-title" id="refresh2"><i class="fa fa-user"> </i> {!! $thread ? $thread->getSubject() : "N/A" !!}</h3>
        <div class="pull-right">
            ...
            <?php if ($group->can_edit_ticket == 1) {?>
                <button type="button" class="btn btn-sm btn-default" data-toggle="modal" data-target="#{{$tickets->id}}change_status"><i class="fa fa-exchange" style="color:teal;"> </i> {!! Lang::get('lang.change_status') !!}</button>
            <?php } ?>
                ...
                <div class="col-md-12">
                    ...
                    <div class="callout callout-{{$priority->priority_color}}" style = 'background-color:{{$priority->priority_color}}; color:#F9F9F9'>
                        <div class="row"> 
                            ...
                            <div class="col-md-3">
                                <b>{!! Lang::get('lang.created_date') !!}: </b> {{ $tickets->created_at }}
                            </div>
                            <div class="col-md-3">
                            ...
                            <div class="col-md-3">
                                ...
                                @endforeach
                                <b>{!! Lang::get('lang.last_response') !!}: </b> {{ $ResponseDate }}
                            </div>
                        </div>
                    </div>
                </div>
                        ...
                        <table class="table table-hover">
                            <div id="refresh">
                                ...
                                @if($user->ban > 0)  <tr><td style="color:orange;"><i class="fa fa-warning"></i><b>
                                    ...
                                    <tr><td><b>Reporting Office:</b></td>   <td>{{$tickets->immigration_office_name }}</td></tr>
                    ...
                    <div class="col-md-6">
                        ...
                        <table class="table table-hover">
                            <div id="refresh3">
                                ...
                                <tr><td><b>{!! Lang::get('lang.ticket') . " " . Lang::get('lang.category') !!}:</b></td> <td title="{{$tickets->category->name}}">{{$tickets->category->name}}</td></tr>
                                <tr><td><b>{!! Lang::get('lang.concluded_category') !!}:</b></td> <td title="{{$tickets->concludedcategory->name ?? 'N/A'}}">{{$tickets->concludedcategory->name ?? 'N/A'}}</td></tr>
                                    ...
                                    <div class="timeline-item">
                                        ...
                                        <h3 class="timeline-header">
                                            ...
                                            <div class="user-block" style="margin-bottom:-5px;margin-top:-2px;">
                                                ...
                                                <span class="description" style="margin-bottom:4px;margin-top:4px;"><i class="fa fa-clock-o"></i> {{$conversation->created_at}}</span>
                ...
                <div class="modal-content">
                    ...
                    <div class="modal-body">
                        ...
                        <div id="assign_body">
                            ...
                            <div class="row">
                                 <div id="Leave a comment" class="col-xs-12 form-group form-field-template">
                                 <label for="Leave a comment">Leave a comment</label>
                                 <textarea id="text-field" name="comments" maxlength="2000" type="textarea" cols="" rows="" class="form-control"></textarea>
                                 </div>
                              </div>
                        </div>
...
<?php if ($group->can_edit_ticket == 1) { ?>
        <!-- Ticket Assign Modal -->
        <div class="modal fade" id="{{$tickets->id}}change_status">
            <div class="modal-dialog">
                <div class="modal-content">
                    <div class="modal-header">
                        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
                        <h4 class="modal-title">{!! Lang::get('lang.change_status') !!}</h4>
                    </div>
                    <div id="assign_alert" class="alert alert-success alert-dismissable" style="display:none;">
                        <button id="assign_dismiss" type="button" class="close" data-dismiss="alert" aria-hidden="true">×</button>
                        <h4><i class="icon fa fa-check"></i>Alert!</h4>
                    ...
                    <div class="modal-body">
                        ...
                        <div id="statuses">
                            <p>{!! Lang::get('lang.status') !!}</p>
                            <select id="status" class="form-control" style="font-family: 'FontAwesome'" name="status">
                                <optgroup label="Status">
                                    <option class="text-danger" value="{!! Lang::get('lang.open') !!}"> &#xf07c; {!! Lang::get('lang.open') !!}</option>
                                    <option class="text-success" value="{!! Lang::get('lang.close') !!}"> &#xf00c; {!! Lang::get('lang.close') !!}</option>
                                    <option class="text-success" value="{!! Lang::get('lang.resolved') !!}"> &#xf058; {!! Lang::get('lang.resolved') !!}</option>
                                </optgroup>
                            </select>
                        </div>
                        
                        <div id="concluded_category_body">
                            <p>{!! Lang::get('lang.concluded_category') !!}</p>
                            <select id="concluded_category" class="form-control" name="concluded_category">
                                <?php
                                    $categories = \App\Model\helpdesk\Ticket\TicketCategory::all();
                                ?>
                    ...
                    <div class="modal-footer">
                        <button type="button" class="btn btn-default pull-left" data-dismiss="modal" id="dismis4">{!! Lang::get('lang.close') !!}</button>
                        <button type="button" class="btn btn-success pull-right" id="change_status">{!! Lang::get('lang.change_status') !!}</button>
                    </div>
                </div><!-- /.modal-content -->
            </div><!-- /.modal-dialog -->
        </div><!-- /.modal -->
                    ...
                    <div class="tab-content">
                        <div class="tab-pane active" id="ahah">
                            <div class="modal-body" id="def">
                                ...
                                <script src="{{asset('lb-faveo/js/jquery-ui.js')}}"></script>  <?php /* <script src="//code.jquery.com/ui/1.11.4/jquery-ui.js"></script> */ ?>
...
<script type="text/javascript">
    ...
    // Close, Reopen or Resolve a Tickets
    var concluded_category_body = document.getElementById('concluded_category_body');
    concluded_category_body.style.display = "none";

    $('#status').on('change', function(e) {
        var status = document.getElementById('status');

        console.log('status changed', status.value)
        if (status.value == 'Close' || status.value == 'Resolved')
            concluded_category_body.style.display = "block";
        else
            concluded_category_body.style.display = "none";
    });
    ...
    $('#change_status').on('click', function() {
        var status = document.getElementById('status');
        var concluded_category = document.getElementById('concluded_category');
        // console.log(status.value, concluded_category.value);
        //return;
        // Close a ticket
        if(status.value == 'Close') {
            ...
            type: "POST",
                    ...
                    data: { concluded_category: concluded_category.value },
                    beforeSend: function() {
                    $("#hidespin").hide();
                            $("#spin").show();
                            $("#hide2").hide();
