 ...
 * Escalation routes
 */
Route::group(['middleware' => PublicAPI::class], function() {
    Route::get('escalate', 'Agent\helpdesk\TicketController@escalate');
});
...
Route::group(['middleware' => ['web']], function () {
    ...
    /**
     * // TODO: Routes for tasks - in progress
     * Routes for tasks
     */
    Route::group(['middleware' => ['auth', Tasks::class]], function() {
        Route::post('/tasks/import', 'Tasks\TasksController@import');
        Route::resource('tasks', 'Tasks\TasksController');
        Route::resource('task-modules', 'Tasks\TaskModulesController');
        Route::resource('task-threads', 'Tasks\TaskThreadsController');
        Route::resource('task-statuses', 'Tasks\TaskStatusesController');
    });
    ...
    Route::group(['middleware' => 'roles', 'middleware' => 'auth', 'middleware' => 'install', 'middleware' => 'update'], function () {
        ...
        Route::get('/teams/add-recipient/{id}', ['as' => 'teams.add_recipient', 'uses' => 'Admin\helpdesk\TeamController@add_recipient']); /*  Get Team View */
        Route::post('/teams/add-recipient/{id}', ['as' => 'teams.add_recipient_store', 'uses' => 'Admin\helpdesk\TeamController@add_recipient_store']); /*  Get Team View */
        Route::patch('/teams/reset-recipient-activate/{id}', ['as' => 'teams.reset_recipient_activate', 'uses' => 'Admin\helpdesk\TeamController@reset_recipient_activate']); /*  Get Team View */
        Route::patch('/teams/reset-recipient-deactivate/{id}', ['as' => 'teams.reset_recipient_deactivate', 'uses' => 'Admin\helpdesk\TeamController@reset_recipient_deactivate']); /*  Get Team View */
        ...
        Route::get('agents/reset-password/{id}', ['as' => 'agents.reset_password', 'uses' => 'Admin\helpdesk\AgentController@reset_password']); 
        Route::patch('agents/reset-password/{id}', ['as' => 'agents.update_reset_password', 'uses' => 'Admin\helpdesk\AgentController@update_reset_password']); 
...
//    Route::post('org-chart-range', ['as' => 'post.org.chart', 'uses' => 'Agent\helpdesk\OrganizationController@orgChartData']);
        ...
        Route::get('ticket-transactions', ['as' => 'report.transactions', 'uses' => 'Agent\helpdesk\TicketTransactionsController@index']);
        
        // Route::get('report/get-stats-ht', ['as' => 'report.get_stats', 'uses' => 'Agent\helpdesk\ReportController@get_stats']); /* Open Tickets */
        Route::get('report/graphical-reports', ['as' => 'report.graphical-reports', 'uses' => 'Agent\helpdesk\ReportController@graphical_reports']); /* Open Tickets */
        Route::get('report/get-graphical-report-data', ['as' => 'report.get-graphical-report-data', 'uses' => 'Agent\helpdesk\ReportController@get_graphical_report_data']); /* Open Tickets */
        Route::get('report/detailed-tickets', ['as' => 'report.open', 'uses' => 'Agent\helpdesk\ReportController@open']); /* Open Tickets */
        Route::get('report/crm-report-overview', ['as' => 'report.crm', 'uses' => 'Agent\helpdesk\Reports\CrmReportController@crm']); /* CRM Report Overview */
        Route::get('report/user/{user_id}', ['as' => 'report.user.show', 'uses' => 'Agent\helpdesk\ReportController@user_show']); /* User Report */
        Route::get('report/user', ['as' => 'report.user', 'uses' => 'Agent\helpdesk\ReportController@user']); /* User Report */
        ...
        Route::post('report-open-ticket-pdf', ['as' => 'report.open.pdf', 'uses' => 'Agent\helpdesk\ReportController@openTicketPdf']);
        Route::post('report-open-ticket-spreadsheet', ['as' => 'report.open.spreadsheet', 'uses' => 'Agent\helpdesk\ReportController@openTicketSpreadsheet']);
        ...
        Route::group(['middleware' => ['auth','force.option', 'role.agent']], function () {
...
//    });
    ...
    Route::get('guest', ['as' => 'guest', 'uses' => 'Client\helpdesk\WelcomepageController@index']);
    Route::get('home', ['as' => 'homee', 'uses' => 'Client\helpdesk\WelcomepageController@index']); //guest layout
...
//    });
    ...
    Route::get('category', ['as' => 'category', 'uses' => 'Agent\kb\CategoryController@index']);

    Route::get('category/create', ['as' => 'category.create', 'uses' => 'Agent\kb\CategoryController@create']);
    Route::post('category/create', ['as' => 'category.store', 'uses' => 'Agent\kb\CategoryController@store']);

    Route::get('category/{id}/edit', 'Agent\kb\CategoryController@edit');
    Route::post('category/{id}/edit', ['as' => 'category.update', 'uses' => 'Agent\kb\CategoryController@update']);
    ...
    Route::get('article', ['as' => 'article', 'uses' => 'Agent\kb\ArticleController@index']);
    Route::post('article/upload', ['as' => 'article.upload', 'uses' => 'Agent\kb\ArticleController@upload']);
    Route::get('article/create', ['as' => 'article.create', 'uses' => 'Agent\kb\ArticleController@create']);
    Route::post('article/create', ['as' => 'article.store', 'uses' => 'Agent\kb\ArticleController@store']);
    Route::get('article/{id}/edit', 'Agent\kb\ArticleController@edit');
    Route::post('article/{id}/edit', ['as' => 'article.update', 'uses' => 'Agent\kb\ArticleController@update']);
    ...
    Route::get('page', ['as' => 'page', 'uses' => 'Agent\kb\PageController@index']);

    Route::get('page/{slug}/edit', 'Agent\kb\PageController@edit');
    Route::post('page/{slug}/edit', ['as' => 'page.update', 'uses' => 'Agent\kb\PageController@update']);
    ...
    Breadcrumbs::register('article-list', function ($breadcrumbs) {
        //$breadcrumbs->parent('/');
        $breadcrumbs->push('Knowledge Base', url('knowledgebase'));
        $breadcrumbs->push('Article List', url('article-list'));
    });
    // /* get search values */
    ...
    Breadcrumbs::register('show', function ($breadcrumbs) {
        ...
        $article = App\Model\kb\Article::where('slug', $slug)->first();
        $all = App\Model\kb\Relationship::where('article_id','=', $article->id)->get();
        ...
        $breadcrumbs->push('Knowledge Base', url('knowledgebase'));
        $breadcrumbs->push('Article List', url('article-list'));
        $breadcrumbs->push($category->name, url('category-list/'.$category->slug));
        $breadcrumbs->push($article->name, url('show/{slug}'));
    ...
    Breadcrumbs::register('category-list', function ($breadcrumbs) {
        //$breadcrumbs->parent('/');
        $breadcrumbs->push('Knowledge Base', url('knowledgebase'));
        $breadcrumbs->push('Category List', url('category-list'));
    });
...
//    Route::get('/reportdata', 'HomeController@pushdata');
    ...
    Route::get('ticket/priority/{id}/description', ['as' => 'priority.description', 'uses' => 'Admin\helpdesk\PriorityController@getDescription']);
