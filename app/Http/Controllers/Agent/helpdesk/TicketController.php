 ...
 */
class TicketController extends TicketTransactionsController
{
    /**
    ...
    public function __construct(PhpMailController $PhpMailController, Notify $NotificationController)
    {
        ...
        $this->middleware('auth')->except(['escalate']);
    }
    ...
    public function newticket(CountryCode $code)
    {
        ...
        return view('themes.default1.agent.helpdesk.ticket.new', compact('email_mandatory', 'settings', 'max_size_in_bytes', 'max_size_in_actual'))->with('phonecode', $pcode);
    }
    ...
    public function post_newticket(CreateTicketRequest $request, CountryCode $code, $api = false)
    {
        try {
            ...
            if ($request->input('immigration_office_id')) {
                $immigration_office_id = $request->input('immigration_office_id');
            } else {
                $immigration_office_id = null;
            }
            ...
            $form_data = $request->except('name', 'phone', 'email', 'subject', 'body', 'helptopic', 'category', '_wysihtml5_mode', '_token', 'mobile', 'code', 'priority', 'attachment', 'first_name', 'last_name', 'sla', 'duedate', 'assignto','immigration_office_id');
            $auto_response = 0;
            $status = 1;
            ...
            //create user
            $result = $this->create_user($email, $fullname, $subject, $body, $phone, $phonecode, $mobile_number, $helptopic, $category, $sla, $priority, $source->id, $headers, $help->department, $assignto, $form_data, $auto_response, $status, $immigration_office_id);
            if ($result[1]) {
                
                // TODO: verify working of this code block
                $ticketNumber = $result[0];
                $ticketId = Tickets::where('ticket_number', $ticketNumber)->first()->id;
                $threadId = Ticket_Thread::where('ticket_id', $ticketId)->first()->id;
                
                // Upload attachments and return response
                $ta = new Ticket_attachments;
                $check_attachment = $this->uploadThreadAttachments($ta, $threadId, $request->file('attachment'));
                if ($check_attachment == 1)
                    $this->attachmentSeperate($threadId);
...
//            dd($tickets);
     ...
     * @return fileName
     */
    public function upload($file, $pathToUpload)
    {
        $fileNameToStore = '';
        ...
        // Storage::putFileAs($pathToUpload, $file, $fileNameToStore);
        $file->storeAs($pathToUpload, $fileNameToStore);
        return $fileNameToStore;
    }

    /**
     * Reusable function to uploads files 
     * from a newly created thread
     * 
     * @param Ticket_attachments  ta
     * @param HttpRequest::file attachments
     * 
     * @return check_attachment
     */
    public function uploadThreadAttachments(Ticket_attachments $ta, $threadId, $attachments)
    {
        $status = null;
        if ($attachments != null) {
            foreach ($attachments as $attachment) {
                if ($attachment != null) {
                    ...
                    $fileName = $this->upload($attachment, 'ticket_attachments');
                    $ta->create([
                        'thread_id' => $threadId, 
                        'name' => $name,
                        'size' => $size,
    ...
    public function reply(Ticket_Thread $thread, Request $request, Ticket_attachments $ta, $mail = true, $system_reply = true, $user_id = '')
    {
        ...
        try {
            ...
            if ($system_reply == true) {
                ...
                if ($tickets->status > 1) {
                    $this->open($ticket_id, new Tickets(), new Request());
                }
            }
            $thread->save();

            // Upload attachments and return response
            $check_attachment = $this->uploadThreadAttachments($ta, $thread->id, $attachments);
    ...
    public function ticket_print($id)
    {
        ...
        $pdf = PDF::loadView('themes.default1.agent.helpdesk.ticket.pdf', $data);
     ...
     * @return type bool
     */
    public function create_user($emailadd, $username, $subject, $body, $phone, $phonecode, $mobile_number, $helptopic, $category, $sla, $priority, $source, $headers, $dept, $assignto, $from_data, $auto_response, $status, $immigration_office_id)
    {
        ...
        $ticket_number = $this->check_ticket($user_id, $subject, $body, $helptopic, $category, $sla, $priority, $source, $headers, $dept, $assignto, $from_data, $status, $immigration_office_id);
        ...
        if ($ticket_number2) {
            ...
            if (Alert::first() && (Alert::first()->ticket_status == 1 || Alert::first()->ticket_admin_email == 1)) {
                // send email to admin
                $admins = User::where('role', '=', 'admin')->where('all_emails', 1)->get();
...
//                Event fire for new ticket['']
     ...
     * @return type string
     */
    public function check_ticket($user_id, $subject, $body, $helptopic, $category, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $immigration_office_id)
    {
        ...
        if (isset($read_ticket_number[1])) {
            ...
            if ($find_number->count() > 0) {
                ...
                $ticket_number = $this->createTicket($user_id, $subject, $body, $helptopic, $category, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $immigration_office_id);
     ...
     * @return type string
     */
    public function createTicket($user_id, $subject, $body, $helptopic, $category, $sla, $priority, $source, $headers, $dept, $assignto, $form_data, $status, $immigration_office_id)
    {
        $ticket_number = '';
        ...
        $ticket->immigration_office_id = $immigration_office_id;
        ...
        $this->saveTransaction($ticket->id, $user_id, $assignto, $status);
    ...
    public function close($id, Tickets $ticket, Request $request)
    {
        ...
        if ($concluded_category = $request->get('concluded_category')) {
            $ticket_status->concluded_category = $concluded_category;
        }

        $ticket_status->status = 3;
        $ticket_status->closed = 1;
        $ticket_status->closed_at = date('Y-m-d H:i:s');
        $ticket_status->reopened = 0;
        $ticket_status->is_deleted = 0;
        $ticket_status->save();
        ...
        // Save this transaction
        $this->saveTransaction($ticket_status->id, $ticket_status->user_id, $this->getLastUserAssigned($ticket_status->id), 3);
...
//        $ticket_status = $ticket->where('id', '=', $id)->first();
        ...
        // Save this transaction
        $this->saveTransaction($ticket_status->id, $ticket_status->user_id, $this->getLastUserAssigned($ticket_status->id), 2);
    ...
    public function open($id, Tickets $ticket, Request $request)
    {
        ...
        $mytime = \Carbon\Carbon::now()->toDateTime();
        $ticket_status->reopened_at = $mytime;
        $ticket_status->reopened = 1;
        $ticket_status->is_deleted = 0;
        $ticket_status->closed = 0;
        $sla_plan = Sla_plan::where('id', '=', $ticket_status->sla)->first();
        $new_date = date_add($mytime, date_interval_create_from_date_string($sla_plan->grace_period));
        $ticket_status->duedate = $new_date;
        $ticket_status->save();
        $ticket_status_message = Ticket_Status::where('id', '=', $ticket_status->status)->first();
        $thread = new Ticket_Thread();
        $thread->ticket_id = $ticket_status->id;
        ...
        // Save this transaction
        $this->saveTransaction($ticket_status->id, $ticket_status->user_id, $this->getLastUserAssigned($ticket_status->id), 1);
    ...
    public function delete($id, Tickets $ticket)
    {
        ...
        if ($ticket_delete->status == 5) {
            ...
            // Save this transaction
            $this->saveTransaction($ticket_delete->id, $ticket_delete->user_id, $this->getLastUserAssigned($ticket_delete->id), 5);
    ...
    public function assign($id)
    {
        ...
        foreach ($ticket_array as $id) {
            ...
            if ($assign_to[0] == 'team') {
                ...
                $thread->user_id = Auth::user()->id;
                $thread->is_internal = 1;
                $msg = 'This Ticket has been assigned to '.$assignee;
                $thread->body = ($comments != '') ? $msg.'.'.$comments : $msg;
                $thread->save();
                ...
                // Save this transaction, If assignee was changed reset ticket to "open"
                $this->saveTransaction($ticket->id, $ticket->user_id, $assign_to[1], 1);
        ...
        if (Input::has('select_all')) {
            ...
            foreach ($selectall as $delete) {
                ...
                if ($value == 'Delete') {
                    ...
                    $this->close($delete, new Tickets(), new Request());
                } elseif ($value == 'Resolve') {
                    $this->resolve($delete, new Tickets(), new Request());
                } elseif ($value == 'Open') {
                    $this->open($delete, new Tickets(), new Request());
            ...
            if (count($overdues) == 0) {
                ...
                foreach ($overdues as $overdue) {
                    ...
                    if (date('Y-m-d H:i:s') > $new_date) {
                        ...
                        // Save this transaction
                        $this->saveTransaction($overdue->id, $overdue->user_id, $this->getLastUserAssigned($overdue->id), 1);
...
//            }
     ...
     * @return repsone/view
     */
    public function getTicketsView(Request $request)
    {
        $assignedTo = null;
        $departmentID = null;
        if(auth()->user()->role == 'agent'){
          $assignedTo = auth()->user()->id;
          $departmentID = auth()->user()->primary_dpt;
        }

        if($request->has('show') &&  ($request->get('show')[0] == 'unanswered')){
            // $tickets = DB::table('unanswered_tickets')->get();
            if (auth()->user()->role == 'agent') {
                $dept = Department::where('id', '=', auth()->user()->primary_dpt)->first();                 
        ...
        }elseif($request->has('show') &&  ($request->get('show')[0] == 'answered')){
            // $tickets = DB::table('answered_tickets')->get();
            if (auth()->user()->role == 'agent') {
                $dept = Department::where('id', '=', auth()->user()->primary_dpt)->first();                 
                $tickets = Tickets::where(function ($query) use ($dept) {
                        $query->where('dept_id', '=', $dept->id)
        ...
        }elseif($request->has('show') &&  ($request->get('show')[0] == 'closed')){
            $tickets = DB::table('closed_tickets')->get();
            // $tickets = Tickets::where('closed', 1)->where('is_deleted', 0)->get();
        }elseif($request->has('show') &&  ($request->get('show')[0] == 'reopened')){
            //   $tickets = DB::table('reopened_tickets')->get();
            if (auth()->user()->role == 'agent') {
                $dept = Department::where('id', '=', auth()->user()->primary_dpt)->first();                 
                $tickets = Tickets::where(function ($query) use ($dept) {
                        $query->where('dept_id', '=', $dept->id)
        ...
        }elseif($request->has('show') &&  ($request->get('show')[0] == 'assigned')){  
            // $tickets = Tickets::where('assigned_to', '!=', NULL)->get();
            $tickets = DB::table('assigned_tickets')->get();
        }elseif($request->has('show') &&  ($request->get('show')[0] == 'overdue')){
            // return date('Y-m-d');
            // $tickets = Tickets::where('status', '=', 1)->where('isanswered', '=', 0)->get();
            $tickets = DB::select(
                DB::raw("CALL spGetOverdueTickets(:departmentID)"),
                [':departmentID' => $departmentID]
              );
  
        }elseif($request->has('show') &&  ($request->get('show')[0] == 'mytickets')){
            // $tickets = DB::select(
            //   DB::raw("CALL spGetMyTickets(:assignedTo)"),
            //   [':assignedTo' => auth()->user()->id]
            // );
        ...
        }elseif($request->has('show') &&  ($request->get('show')[0] == 'inbox') && $request->has('departments') && ($request->get('departments')[0] == 'All')){
            // $tickets = DB::select(
            //     DB::raw("CALL spGetInboxTickets(:departmentID, :assignedTo)"),
            //     [':departmentID' => $departmentID, ':assignedTo' => $assignedTo]
            // );
        ...
        }elseif($request->has('show') && ($request->get('show')[0] == 'unassigned')){
        //   $tickets = Tickets::where('assigned_to', NULL)->get();
            $tickets = DB::select(
                DB::raw("CALL spGetUnassignedTickets(:departmentID)"),
                [':departmentID' => $departmentID]
            );

        }elseif($request->has('show') && ($request->get('show')[0] == 'trash')){
        //   $tickets = Tickets::where('is_deleted', 1)->get();
        $tickets = DB::select(
            DB::raw("CALL spGetDeletedTickets(:departmentID)"),
            [':departmentID' => $departmentID]
          );
        }elseif($request->has('show') && ($request->get('show')[0] == 'due_today')){
          $tickets = DB::select(
            DB::raw("CALL spGetDueTodayTickets(:departmentID)"),
            [':departmentID' => $departmentID]
          );
        }elseif($request->has('show') && ($request->get('show')[0] == 'escalated')){
            if (auth()->user()->management_level == 1)
                return redirect()->route('dashboard');
            $tickets = DB::table('ticket_thread as th')
                ->join('tickets as t', 't.id', '=', 'th.ticket_id')
                ->join('users as usr', 'usr.id', '=', 't.user_id')
                ->join('ticket_priority as ticp', 'ticp.priority_id', '=', 't.priority_id')
                ->join('ticket_status as tics', 'tics.id', '=', 't.status')
                ->leftJoin('users as usrr', 'usrr.id', '=', 't.assigned_to')
                ->selectRaw('
                    th.title as ticket_subject, 
                    t.*,
                    concat(usr.first_name, " ", usr.last_name, " <", usr.email, "> ") as owner,
                    concat(usrr.first_name, " ", usrr.last_name, " <", usrr.email, "> ") as assigned_to,
                    th.created_at as last_activity,
                    ticp.priority_color as priority_color,
                    ticp.priority as priority
                ')
                ->where('t.escalated', '!=', 1)
                ->where('t.escalated', '=', auth()->user()->management_level)
                ->groupBy('t.id')
                ->orderBy('th.created_at', 'desc')
                ->get();
          }

        //if($request->has('departments') &&  ($request->get('departments')[0] == auth()->user()->primary_department_name ) ){
        if($request->has('departments') &&  ($request->get('departments')[0] != '' ) ){
           // return response()->json($request->all());
           $dept_name = $request->get('departments')[0];
           if ($request->has('status') &&  ($request->get('status')[0] == 'Closed')){
            //    $tickets = Tickets::join('department as dept', 'dept.id', '=', 'dept_id')
            //                 ->where('dept.name', $request->get('departments')[0])
            //                 ->where('closed', 1)
            //                 ->where('is_deleted', 0)
            //                 ->get();
            $tickets = DB::select(
                DB::raw("CALL spGetClosedTicketsPerDept(:dept_name)"),
                [':dept_name' => $dept_name]
              );

           }elseif ($request->has('status') &&  ($request->get('status')[0] == 'Open')){
                // $tickets = Tickets::join('department as dept', 'dept.id', '=', 'dept_id')
                //             ->where('dept.name', $request->get('departments')[0])
                //             ->where('isanswered', 0)
                //             ->where('closed', 0)
           ...
           }elseif ($request->has('status') &&  ($request->get('status')[0] == 'Resolved')){
                // $tickets = Tickets::join('department as dept', 'dept.id', '=', 'dept_id')
                //             ->where('dept.name', $request->get('departments')[0])
                //             ->where('isanswered', 1)
                //             ->where('closed', 1)
                ...
                //             ->get();
                $tickets = DB::select(
                    DB::raw("CALL spGetResolvedTicketsPerDept(:dept_name)"),
                    [':dept_name' => $dept_name]
                  );
   
           }elseif ($request->has('status') &&  ($request->get('status')[0] == 'Deleted')){
                // $tickets = Tickets::join('department as dept', 'dept.id', '=', 'dept_id')
                //             ->where('dept.name', $request->get('departments')[0])
                //             ->where('is_deleted', 1)
                //             ->get();
                $tickets = DB::select(
                    DB::raw("CALL spGetDeletedTicketsPerDept(:dept_name)"),
                    [':dept_name' => $dept_name]
                  );
        ...
        $data['tickets'] = $tickets;
        return view('themes.default1.agent.helpdesk.ticket.tickets', $data);
    }
    ...
    /**
     * Escalate Tickets - default policy => [duedate]
     */
    public function escalate(Request $request)
    {
        if ($request->has("policy")) {
            if (strtolower($request->get("policy")) == "duedate")
                return $this->escalateTicketsByDueDate();
            else if (strtolower($request->get("policy")) == "sla")
                return $this->escalateTicketsBySLA();
            else 
                return response()->json(["message" => "escalation policy should be one of the following [sla, duedate]"], 400);
        }
        return $this->escalateTicketsByDueDate();
    }

    /**
     * Run periodically to check tickets to be escalated
     */
    static function escalateTicketsByDueDate()
    {
        $updated_due_date = [];
        // Escalated tickets with due date
        for ($level = 1; $level <= 3; $level++) {
            $ticketIds = DB::table('tickets')
                ->selectRaw("
                    id,
                    time_to_sec(timediff(now(), duedate )) / 3600 as time_diff, 
                    duedate as due_date,
                    now() as curr_date,
                    (case 
                        when time_to_sec(timediff(now(), duedate )) / 3600 <= 24 then 1
                        when (time_to_sec(timediff(now(), duedate )) / 3600 > 24 and time_to_sec(timediff(now(), duedate )) / 3600 < 74) then 2
                        else 3
                    end) as escalated_level
                ")
                ->havingRaw('escalated_level = ' . $level)
                ->get()
                ->pluck("id")
                ->toArray();

            array_push($updated_due_date,
                DB::table('tickets')
                    ->selectRaw("id")
        ...
        return [
                "policy" => "Due Date",
                "updated" => $updated_due_date,
        ];
    }

    static function escalateTicketsBySLA()
    {
        $updated_sla = [];
        // Escalated tickets with sla
        for ($level = 1; $level <= 3; $level++) {
            $ticketIds = DB::table('tickets')
                ->selectRaw("
                    time_to_sec(timediff(now(), created_at )) / 3600 as time_diff, 
                    created_at as issued_date, 
                    now() as curr_date,
                    (case
                        when time_to_sec(timediff(now(), created_at )) / 3600 <= 24 then 1
                        when (time_to_sec(timediff(now(), created_at )) / 3600 > 24 and time_to_sec(timediff(now(), created_at )) / 3600 < 48) then 2
                        else 3
                    end) as escalated_level
                ")
                ->havingRaw('escalated_level = ' . $level)
