...
class PhpMailController extends Controller
{
    ...
    public function sendmail($from, $to, $message, $template_variables)
    {
        ...
        $job = (new \App\Jobs\SendEmail($from, $to, $message, $template_variables))->delay(\Carbon\Carbon::now()->addMinutes(2));
    ...
    public function setQueue()
    {
        ...
        $active_queue = $queue->where('short_name', env('QUEUE_DRIVER', 'database'))->first();

        // dd($queue->where('status', 1)->get());
        if ($active_queue) {
