...
use App\Model\helpdesk\Agent_panel\Organization as Organization;
// model
...
class AgentController extends Controller
{
    ...
    public function create(Timezones $timezone, Groups $group, Department $department, Teams $team_all, CountryCode $code)
    {
        try {
            ...
            return view('themes.default1.admin.helpdesk.agent.agents.create', compact('assign', 'teams', 'agents', 'timezones', 'groups', 'organizations', 'departments', 'team', 'send_otp'))->with('phonecode', $phonecode->phonecode);
        } catch (Exception $e) {
    ...
    public function store(User $user, AgentRequest $request)
    {
        ...
        $user->fill($request->except(['group', 'primary_department', 'agent_time_zone', 'mobile', 'organization']))->save();
        ...
        $user->all_emails = $request->all_emails;
        ...
        $user->is_tasks_admin = $request->is_tasks_admin;
        $user->can_view_tasks = $request->can_view_tasks;
    ...
    public function edit($id, User $user, Assign_team_agent $team_assign_agent, Timezones $timezone, Groups $group, Department $department, Teams $team, CountryCode $code)
    {
        try {
            ...
            return view('themes.default1.admin.helpdesk.agent.agents.edit', compact('teams', 'assign', 'table', 'teams1', 'selectedTeams', 'user', 'timezones', 'groups', 'organizations', 'departments', 'team', 'exp', 'counted'))->with('phonecode', $phonecode->phonecode);
        } catch (Exception $e) {
            // dd($e);
    ...
    public function reset_password($id, User $user, Assign_team_agent $team_assign_agent, Timezones $timezone, Groups $group, Department $department, Teams $team, CountryCode $code)
    {
        try {
            $location = GeoIP::getLocation();
            $phonecode = $code->where('iso', '=', $location->iso_code)->first();
            ...
            // compact('teams', 'assign', 'table', 'teams1', 'selectedTeams', 'user', 'timezones', 'groups', 'departments', 'team', 'exp', 'counted'))

            return view('themes.default1.admin.helpdesk.agent.agents.reset-password', 
                // $data)
                compact('teams', 'assign', 'table', 'teams1', 'selectedTeams', 'user', 'timezones', 'groups', 'departments', 'team', 'exp', 'counted'))
    ...
    public function update_reset_password($id, User $user, Request $request)
    {
        $request->validate(
            [
        ...
        try {
            ...
            return redirect('agents')->with('success', 'Agent Password Updated Successfully');
        } catch (Exception $e) {
    ...
    public function update($id, User $user, AgentUpdate $request, Assign_team_agent $team_assign_agent)
    {
        ...
        $management_level = $request->input('management_level');
        $is_tasks_admin = $request->input('is_tasks_admin');
        ...
        try {
            ...
            $user->is_tasks_admin = $is_tasks_admin;
            $user->can_view_tasks = $request->can_view_tasks;
            $user->agent_tzone = $request->agent_time_zone;
            $user->save();
