...
use App\Model\helpdesk\Agent\TeamAssignment;
...
class TeamController extends Controller
{
    ...
    public function show($id, User $user, Assign_team_agent $assign_team_agent, Teams $team)
    {
        try {
            ...
            // $teamDetails = \App\Model\helpdesk\Agent\Teams::whereId($id)->first();

            // dd($teams->getTeamMembers());
            ...
            $groupRecipients = TeamAssignment::where('hide',0)->where('from_team_id',$teams->id)->get();

            return view('themes.default1.admin.helpdesk.agent.teams.show', compact('user', 'teams', 'id', 'groupRecipients'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }
    ...
    public function add_recipient($id, User $user, Teams $team)
    {
        try {
            $teams = $team->whereId($id)->first();

            $allTeams = Teams::where('status', 1)->where('id','<>', $id);

            $user = $user->where('role', '<>', 'user')->where('active', '=', 1)->orderBy('first_name')->get();

            return view('themes.default1.admin.helpdesk.agent.teams.add_recipients', compact('user','teams','allTeams'));
        } catch (Exception $e) {
            return redirect()->back()->with('fails', $e->getMessage());
        }
    }

    public function add_recipient_store($id,User $user, Teams $team, Request $request)
    {
        $request->validate(
            [
            ...
            [
                'to_team_id.required' => 'The Recipient Team Field is Required',
                'to_team_id.exists' => 'The Recipient Team Does Not Exist',
            ]
        );
        ...
        try {
            ...
            $assignment = TeamAssignment::firstOrCreate(
                [
                    'id' => $id,
    ...
    public function reset_recipient_activate($id ,User $user, Teams $team, Request $request)
    {
        // dd('activate');
        try {
            $user = $user->where('role', '<>', 'user')->where('active', '=', 1)->orderBy('first_name')->get();
            ...
            $assignment = TeamAssignment::where('id', $id)->first();
            $assignment->active = 1;
            $assignment->save();
    ...
    public function reset_recipient_deactivate($id,User $user, Teams $team, Request $request)
    {
        try {
            $user = $user->where('role', '<>', 'user')->where('active', '=', 1)->orderBy('first_name')->get();
            $teams = $team->whereId($id)->first();
