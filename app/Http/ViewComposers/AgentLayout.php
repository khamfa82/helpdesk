...
class AgentLayout
{
    ...
    //     {
    //        $dept_id = $this->auth->primary_dpt;
    //        $assigned_to = $this->auth->id;
    //     }

    //    $sql = DB::select(DB::raw("CALL spGetTickets(:departmentID, :assignedTo)"),
    //   [':departmentID' => $dept_id, ':assignedTo' => $assigned_to]);

    //    return DB::select(DB::raw("CALL spGetTickets(:departmentID, :assignedTo)"),
    //   [':departmentID' => $dept_id, ':assignedTo' => $assigned_to]);
    ...
    public function myTicket()
    {
        ...
        $assigned_to = null;
        if($this->auth->role == 'agent' OR $this->auth->role == 'admin' )
          $assigned_to = $this->auth->id;

       $sql = DB::select(DB::raw("CALL spGetMyTickets(:assignedTo)"),
    ...
    public function unassigned()
    {
       ...
       $sql = DB::select(DB::raw("CALL spGetUnassignedTickets(:departmentID)"),
    ...
    public function followupTicket()
    {
        ...
        if($this->auth->role == 'agent' OR $this->auth->role == 'admin')
        {
          $sql = DB::table('follow_up_tickets')->get();
          return $sql;
        }
    ...
    public function deleted()
    {
        ...
        $sql = DB::select(DB::raw("CALL spGetDeletedTickets(:departmentID)"),
          [':departmentID' => $dept_id]);
    ...
    //    $sql = DB::select(DB::raw("CALL spGetInboxTickets(:departmentID, :assignedTo)"),
    //   [':departmentID' => $dept_id, ':assignedTo' => $assigned_to]);
    ...
    //     if (Auth::user()->role == 'agent') {
    //         $dept = Department::where('id', '=', Auth::user()->primary_dpt)->first();                 
    //         $tickets = Tickets::where('dept_id', '=', $dept->id)
    //             ->orWhere('assigned_to', Auth::user()->id)
    //             ->get();
    //     } elseif (Auth::user()->role == 'admin') {
    //         $tickets = Tickets::where('status', 1)
    ...
    public function overdues()
    {
       ...
       $sql = DB::select(DB::raw("CALL spGetOverdueTickets(:departmentID)"),
      [':departmentID' => $dept_id]);
      ...
      //dd($sql, count($sql));
       ...
       return $sql;
        ...
        //return DB::select('exec my_stored_procedure(?,?,..)',array($Param1,$param2));
    }
    ...
    public function getDueToday()
    {
       ...
       $sql = DB::select(DB::raw("CALL spGetDueTodayTickets(:departmentID)"),
         [':departmentID' => $dept_id]);
